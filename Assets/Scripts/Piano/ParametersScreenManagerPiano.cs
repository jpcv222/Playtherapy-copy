...
public class ParametersScreenManagerPiano : MonoBehaviour, IParametersManager
{
    ...
    List<bool> valoresL;
    List<bool> valoresR;
    List<InputField> arrayInputField;

    private void Start()
    {
        leftFingers = new List<Leap.Finger.FingerType>();
        rightFingers = new List<Leap.Finger.FingerType>();
        valoresL = new List<bool>() { true, true, true, true };
        valoresR = new List<bool>() { true, true, true, true };
        arrayInputField = new List<InputField>() { inputFieldIndexAngle, inputFieldMiddleAngle, inputFieldRingAngle, inputFieldPinkyAngle };
    ...
    */
    ...
    public void OnToggleFingerValueChanged()
    {
        ...
        for (int i = 0; i < togglesFingers.Length / 2; i++)
        {
            ...
            {
                valoresL[i] = false;
                if (valoresL[i-1]== false && valoresR[i-1]== false)
                {
                    arrayInputField[i - 1].enabled = false;
                    arrayInputField[(i - 1) / 2].image.color = Color.gray;

                }
                else
                {
                    arrayInputField[(i - 1) / 2].enabled = true;
                    arrayInputField[(i - 1) / 2].image.color = Color.white;
        ...
        for (int i = togglesFingers.Length / 2; i < togglesFingers.Length; i++)
        {
            ...
            {
                valoresR[(i-1)/2] = false;
                if (valoresL[(i-1)/2] == false && valoresR[(i-1)/2] == false)
                {
                    arrayInputField[(i - 1)/2].enabled = false;
                    arrayInputField[(i - 1) / 2].image.color = Color.gray;
                }
                else
                {
